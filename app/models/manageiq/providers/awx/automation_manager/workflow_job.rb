class ManageIQ::Providers::Awx::AutomationManager::WorkflowJob <
  ...
  def raw_status
    ...
    msg = "AWX Workflow Job #{name} with id(#{id}) does not exist on #{ext_management_system.name}"
    raise MiqException::MiqOrchestrationStackNotExistError, msg
  rescue StandardError => err
    _log.error("AWX Workflow Job #{name} with id(#{id}) status error: #{err}")
  ...
  def update_child_job(raw_job)
    ...
    unless job
      ...
      job = ManageIQ::Providers::Awx::AutomationManager::Job.create(
  ...
  def update_child_workflow_job(raw_workflow_job)
    ...
    unless job
      ...
      job = ManageIQ::Providers::Awx::AutomationManager::WorkflowJob.create(
