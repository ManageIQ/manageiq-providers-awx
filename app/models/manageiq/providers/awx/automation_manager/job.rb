...
class ManageIQ::Providers::Awx::AutomationManager::Job <
  ...
  def raw_status
    ...
    _log.error "Awx Job #{name} with id(#{id}) status error: #{err}"
  ...
  def raw_stdout(format = 'txt')
    ...
    _log.error "Reading Awx Job #{name} with id(#{id}) stdout failed with error: #{err}"
    raise MiqException::MiqOrchestrationStatusError, err.to_s, err.backtrace
  end
  ...
  def raw_artifacts
    ...
    msg = "Awx Job #{name} with id(#{id}) or its artifacts does not exist on #{ext_management_system.name}"
    raise MiqException::MiqOrchestrationStackNotExistError, msg
  rescue => err
    _log.error("Reading Awx Job #{name} with id(#{id}) artifacts failed with error: #{err}")
    raise MiqException::MiqOrchestrationStatusError, err.to_s, err.backtrace
  end
