...
describe ManageIQ::Providers::Awx::AutomationManager::ConfigurationScriptSource do
  let(:manager) do
  ...
  let(:credential)    { FactoryBot.create(:awx_scm_credential, :manager_ref => '1') }
  ...
  context "Update through API" do
    ...
    let(:project)       { described_class.create!(:manager => manager, :manager_ref => tower_project.id) }
    let(:tower_cred)    { FactoryBot.create(:awx_scm_credential, :manager_ref => '100') }
