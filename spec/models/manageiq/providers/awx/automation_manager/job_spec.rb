...
describe ManageIQ::Providers::Awx::AutomationManager::Job do
  let(:job) { FactoryBot.create(:awx_job) }
  ...
  let(:provider) { FactoryBot.create(:provider_awx) }
  let(:manager)  { FactoryBot.create(:automation_manager_awx, :provider => provider) }
  ...
  let(:cloud_credential)   { FactoryBot.create(:awx_cloud_credential,   :manager_ref => '2', :resource => manager) }
  let(:network_credential) { FactoryBot.create(:awx_network_credential, :manager_ref => '3', :resource => manager) }
  let(:vault_credential)   { FactoryBot.create(:awx_vault_credential,   :manager_ref => '4', :resource => manager) }
  ...
  let(:template) { FactoryBot.create(:awx_configuration_script, :manager => manager) }
  subject { FactoryBot.create(:awx_job, :job_template => template, :ext_management_system => manager) }
  ...
  describe 'job operations' do
    describe ".create_job" do
      ...
      context 'template is temporary' do
        let(:template) { FactoryBot.build(:awx_configuration_script, :manager => manager) }
      ...
      context 'options have extra_vars' do
        let(:template) do
          FactoryBot.build(:awx_configuration_script,
                            :manager     => manager,
    ...
    context "#refres_ems" do
      before do
        allow_any_instance_of(ManageIQ::Providers::Awx::Provider).to receive_messages(:connect => connection)
      end
